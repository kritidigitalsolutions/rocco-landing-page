<?php
/**
 * Rocco Play Admin — Edit Popular Category
 */
$pageTitle = 'Edit Popular Category';
$breadcrumb = [
...
$stmt = $pdo->prepare("SELECT * FROM popular_categories WHERE id = ?");
$stmt->execute([$id]);
$category = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("UPDATE popular_categories SET 
                               name = :name, 
        ...
        $_SESSION['flash'] = ['type' => 'success', 'message' => 'Popular Category "' . $name . '" updated successfully!'];
        header('Location: popular_categories.php');
        exit;
    }
}
...
?>
      ...
      <div class="page-header">
        <h1 class="page-title">Edit Popular Category</h1>
        <p class="page-subtitle">Update the popular category card for the homepage.</p>
      </div>
      ...
      <div class="card" style="max-width:700px;">
        ...
        <form method="POST" action="popular_category_edit.php?id=<?php echo $id; ?>" enctype="multipart/form-data" id="popularCategoryForm">
          <?php echo csrfField(); ?>
          ...
          <div class="form-group">
            ...
            <input type="text" name="name" class="form-control-admin" placeholder="e.g. Action" 
                   value="<?php echo htmlspecialchars($_POST['name'] ?? $category['name']); ?>" required id="catName">
          </div>
          ...
          <div class="form-group">
            ...
            <input type="text" name="titles_count" class="form-control-admin" placeholder="e.g. 1,200+ titles" 
                   value="<?php echo htmlspecialchars($_POST['titles_count'] ?? $category['titles_count']); ?>">
            ...
            <div style="display:flex;gap:12px;align-items:flex-start;">
              <div style="flex:1;">
                <input type="text" name="icon_class" id="iconClassInput" class="form-control-admin" 
                       ...
                       value="<?php echo htmlspecialchars($_POST['icon_class'] ?? ($category['icon_class'] ?? '')); ?>">
              ...
              <?php $currentIcon = $_POST['icon_class'] ?? ($category['icon_class'] ?? ''); ?>
              <div id="iconPreviewBox" style="width:56px;height:56px;min-width:56px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.05);border:2px solid <?php echo !empty($currentIcon) ? 'rgba(212,175,55,0.4)' : 'rgba(255,255,255,0.08)'; ?>;border-radius:12px;font-size:1.5rem;color:var(--brand-gold);transition:all 0.3s ease;">
                <i id="iconPreview" class="<?php echo htmlspecialchars(!empty($currentIcon) ? $currentIcon : 'fas fa-icons'); ?>" style="opacity:<?php echo !empty($currentIcon) ? '1' : '0.5'; ?>;"></i>
              </div>
            </div>
          </div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
            <div class="form-group">
              ...
              <input type="number" name="display_order" class="form-control-admin" value="<?php echo htmlspecialchars($_POST['display_order'] ?? $category['display_order']); ?>" min="0">
            </div>
            <div class="form-group">
              <label class="form-label">Card Color</label>
              <input type="color" name="card_color" class="form-control-admin" style="padding: 2px" 
                     value="<?php echo htmlspecialchars($_POST['card_color'] ?? $category['card_color']); ?>">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
            <div class="form-group">
              ...
              <?php if ($category['bg_image']): ?>
                 <div style="margin-top:8px;">
                     <img src="../<?php echo htmlspecialchars($category['bg_image']); ?>" style="max-height:80px;border-radius:4px;border:1px solid #444;" alt="bg">
                 </div>
              <?php endif; ?>
            </div>
            <div class="form-group">
              ...
              <?php if ($category['icon_image']): ?>
                 <div style="margin-top:8px;">
                     <img src="../<?php echo htmlspecialchars($category['icon_image']); ?>" style="max-height:80px;border-radius:4px;border:1px solid #444;background:#333;" alt="icon">
                 </div>
              <?php endif; ?>
          ...
          <div class="form-group">
            <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding-top:6px;">
              <input type="checkbox" name="is_icon_hidden" value="1" <?php echo (!empty($_POST['is_icon_hidden']) || $category['is_icon_hidden']) ? 'checked' : ''; ?> style="accent-color:var(--brand-red-bright);">
